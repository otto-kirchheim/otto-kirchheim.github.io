...
| `data:selectYear` | `userLoginSuccess.ts` → `selectYear()` → `loadUserDaten.ts` | Fetches and populates year data from server/storage; also calls `syncFeatureTabs(aktivierteTabs)` (`app/init/syncFeatureTabs.ts`), which mounts/unmounts the Bereitschaft/EWT/Neben Preact tab content via `featureLifecycleRegistry` — re-evaluated on every year/month change, not just login |
...
(`app/init/syncFeatureTabs.ts`) so the next login's `syncFeatureTabs` call doesn't think a
