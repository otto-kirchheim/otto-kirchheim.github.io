  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style="width:100%;height:clamp(80px,15vw,160px);display:block" aria-hidden="true">
      ...
      {nonPeriodic.map((p, i) => {
        ...
        return (
          <line key={i} x1={x} y1={PAD} x2={x} y2={H - PAD}
            stroke={EVENT_COLORS[p.event]} strokeWidth="1.5" strokeDasharray="4,3" opacity="0.8" />
        );
      })}
      ...
      <text x={PAD + 2} y={PAD + 8} fontSize="7" fill="var(--bs-body-color)" opacity="0.5">{vMax} MB</text>
    </svg>
  );
}
...
function MemoryCard({
  ...
  const lastSnap = (heap?.history.length ?? 0) > 0
    ? heap!.history[heap!.history.length - 1]
    : null;
      ...
      <div class="card-body">
        ...
        <div class="d-flex align-items-center justify-content-between mb-2 gap-2">
          <h6 class="card-title fw-semibold mb-0 text-nowrap">
            <span class="material-icons-round me-1" style="font-size:1rem;vertical-align:middle">memory</span>
            Memory-Verlauf
            <span class="text-body-secondary fw-normal ms-1" style="font-size:.75rem">(7 Tage)</span>
          </h6>
          <div class="d-flex gap-2 flex-shrink-0">
            <button class="btn btn-sm btn-outline-success" onClick={takeSnapshot} disabled={snapping || loading}
              title="Manuellen Heap-Snapshot jetzt speichern">
              {snapping
                ? <span class="spinner-border spinner-border-sm" />
                : <span class="material-icons-round" style="font-size:1rem;vertical-align:middle">add_chart</span>}
            </button>
        ...
        {loading && !heap
          ? <div class="text-center py-3"><span class="spinner-border spinner-border-sm text-primary" /></div>
          : (
            <>
              ...
              {cur && (
                <p class="small text-body-secondary mb-1">
                  ...
                  {' · '}
                  <span class="fw-semibold" style="color:var(--bs-orange)">RSS</span> {cur.rss} MB
                  {' · '}Extern {cur.external} MB
                  {lastSnap && (
                    <>{' · '}Loop {lastSnap.eventLoopDelay} ms · Uptime {formatUptime(lastSnap.uptime).value} {formatUptime(lastSnap.uptime).unit}</>
                  )}
                </p>
              )}
              ...
              <div class="d-flex gap-2 mt-1 flex-wrap" style="font-size:.72rem;color:var(--bs-secondary-color)">
                ...
                ] as [string, boolean, string][]).map(([color, dashed, label]) => (
                  <span key={label} class="d-flex align-items-center gap-1">
                    <span style={`width:14px;height:${dashed ? '0' : '2px'};background:${dashed ? 'none' : color};border-top:${dashed ? `2px dashed ${color}` : 'none'};opacity:.85;display:inline-block;flex-shrink:0`} />
                    {label}
                  </span>
                ))}
              </div>
              ...
              {nonPeriodic.length > 0 && (
                <div class="mt-2 pt-2 border-top">
                  <div class="small text-body-secondary mb-1">Ereignisse:</div>
                  <ul class="list-unstyled mb-0">
                    {nonPeriodic.map((p, i) => {
                      const icon = p.event === 'startup' ? 'power_settings_new'
                        : p.event === 'shutdown' ? 'power_off'
                        : 'add_chart';
                      ...
                      return (
                        <li key={i} class="py-1 border-bottom">
                          <div class="d-flex align-items-center gap-2">
                            <span class="material-icons-round flex-shrink-0"
                              style={`font-size:.85rem;color:${EVENT_COLORS[p.event]}`}>{icon}</span>
        ...
        <div class="col-md-4">
          <div class="card border-0 shadow-sm h-100">
            <div class="card-body">
              ...
              {(
                [
                  ['Bereitschaftseinsätze', stats.resources.bereitschaftseinsaetze, stats.growth.bereitschaftseinsaetzeLast7d],
                  ['Bereitschaftszeiträume', stats.resources.bereitschaftszeitraeume, stats.growth.bereitschaftszaetraumeLast7d],
                ...
                <div key={label} class="d-flex justify-content-between align-items-start py-2 border-bottom gap-2">
                  <span class="small" style="min-width:0;word-break:break-word">
                    {label}
                    {growth > 0 && (
                      <span class="badge bg-success-subtle text-success ms-1">+{growth}</span>
                    )}
                  </span>
                  <span class="badge bg-primary rounded-pill flex-shrink-0">{count.toLocaleString()}</span>
                </div>
              ))}
              <div class="text-body-secondary mt-2" style="font-size:.7rem">+N = neue Einträge (7 Tage)</div>
            </div>
          </div>
        </div>
              ...
              {(
                ...
                <div key={label} class="d-flex justify-content-between align-items-center py-2 border-bottom">
                  <span class="d-flex align-items-center gap-2 small">
                    <span class={`material-icons-round ${color}`} style="font-size:1rem">{icon}</span>
                    {label}
                  </span>
                  <span class="badge bg-secondary rounded-pill">{count}</span>
                </div>
