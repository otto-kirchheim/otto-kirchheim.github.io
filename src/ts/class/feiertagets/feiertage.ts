 ...
 */
export function addTranslation(isoCode: string, newTranslation: TranslationTable): void {
...
export function getHolidayByDate(date: Date, region: Region = "ALL"): Holiday | void {
	checkRegion(region);
...
export function isSpecificHoliday(date: Date, holidayName: HolidayType, region: Region = "ALL"): boolean {
	checkRegion(region);
...
function getHolidaysOfYear(year: number, region: Region): Holiday[] {
	...
	return holidays.sort((a: Holiday, b: Holiday) => a.date.getTime() - b.date.getTime());
}
...
function addRegionalHolidays(year: number, region: Region, feiertageObjects: Holiday[]) {
	if (region === "AUGSBURG") {
		feiertageObjects.push(newHoliday("AUGSBURGER_FRIEDENSFEST", makeDate(year, 8, 8)));
	}
}
function addHeiligeDreiKoenige(year: number, region: Region, feiertageObjects: Holiday[]): void {
...
function addFronleichnam(region: Region, easterDate: Date, holidays: Holiday[]): void {
	if (
		region === "BW" ||
		region === "BY" ||
		region === "AUGSBURG" ||
...
function addMariaeHimmelfahrt(year: number, region: Region, holidays: Holiday[]): void {
	if (region === "SL" || region === "BY" || region === "AUGSBURG" || region === "ALL") {
		holidays.push(newHoliday("MARIAHIMMELFAHRT", makeDate(year, 8, 15)));
	}
}

function addReformationstag(year: number, region: Region, holidays: Holiday[]): void {
	if (
		year === 2017 ||
		region === "NI" ||
		region === "BB" ||
...
function addAllerheiligen(year: number, region: Region, holidays: Holiday[]): void {
	if (
		region === "BW" ||
		region === "BY" ||
		region === "AUGSBURG" ||
...
function addBussUndBetttag(year: number, region: Region, holidays: Holiday[]): void {
	if (region === "SN" || region === "ALL") {
		// @todo write test
		const bussbettag = getBussBettag(year);
		holidays.push(
			newHoliday("BUBETAG", makeDate(bussbettag.getUTCFullYear(), bussbettag.getUTCMonth() + 1, bussbettag.getUTCDate()))
		);
	}
}

function addWeltkindertag(year: number, region: Region, holidays: Holiday[]): void {
	if (year >= 2019 && (region === "TH" || region === "ALL")) {
		holidays.push(newHoliday("WELTKINDERTAG", makeDate(year, 9, 20)));
	}
}

function addWeltfrauenTag(year: number, region: Region, feiertageObjects: Holiday[]): void {
	if (year <= 2018) {
		return;
	}
