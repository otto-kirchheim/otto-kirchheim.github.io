...
import type { IDatenEA, IDatenEWT } from '@/shared/types';
...
import syncEaDurationFromEwtRows from '@/features/ea/model/syncEwtToEa';
...
function mountTableEA(rows: (IDatenEA & { _state?: string })[]) {
  ...
  const patchCellsAsModified = (transform: (row: (typeof tableRows)[number]) => IDatenEA | null): boolean => {
    let changed = false;
