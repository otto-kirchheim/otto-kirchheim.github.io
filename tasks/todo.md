...
- [ ] Hilfetexte inhaltlich gegen die heutige UI abgleichen (User-Hinweis nach Browser-Check P1h, 2026-09-20): `tab.einstellungen` nennt "Logout"/"Passwort ändern" als Buttons, die UI hat aber Abschnitte (Sicherheit, Biometrie), Speichern, Auswählen; weitere Hilfen (Tabs, Dialoge) einzeln pruefen. Reiner Inhalts-Commit, kein Code (Texte liegen in `core/help/helpContent.ts` und `features/<Ordner>/parts/help.ts`); Liste der betroffenen Hilfen vom User abwarten.
- [ ] P1i (eigener Commit nach P1h, vom User beauftragt 2026-09-20) `datenKatalog.ts` nach Feature aufteilen: Felder, Zeilenquellen (`ZEILEN_FELDER`/`ZEILEN_QUELLEN`) und Listen-Vorlagen (`LISTEN_VORLAGEN`/`EZ_LISTEN`/`VORLAGEN_KATEGORIE`) je Admin-Feature-Ordner (`features/Admin/features/{ber,ewt,ez,ea}/`), `datenKatalog.ts` behaelt `BASIS`, Schriftarten, Formate, Helfer und setzt den Rest aus dem Admin-Manifest zusammen; `FormularCode` nicht mehr fest. Tests fuer FormularEditor-Konsumenten, Katalog je Teilmenge; danach Scaffold um Admin-Ordner erweitern.
