...
- [ ] P1c Ressourcen-Meta & Daten für alle vier Features (ber, ewt, ez, ea): `resourceConfig`, `Storage`, `persist*`, `autoSave/*`, `loadUserDaten.*`, `overwriteUserDaten`, `changeMonatJahr`, `fieldMapper`, `actAs` aus `meta` bzw. Slot `data` (EA hat `meta`+`parts` seit P1a, bekommt hier ebenfalls seine Ressourcen/Daten-Slots; die Tab-Migration von ber/ewt/ez ist seit P1b erledigt)
