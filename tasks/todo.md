...
- [ ] P1c-2 API-Adapter und Monat/Jahr: `savePipeline` (Dispatch + EWT-Link-Bereinigung -> Event), `changeTracking`, `dataApi`, `fieldMapper`, `apiService.loadAllYearData`, `saveDaten.getButtonResources`, `setMonatJahr` -> Monat/Jahr-Store bzw. `period:changed` (Ueberschriften ohne DOM-Schreiben)
