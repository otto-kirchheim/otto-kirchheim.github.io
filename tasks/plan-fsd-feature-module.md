...
- **P1 Feature-Contract & Module (XL, keine Moves, In-place, Verhaltensänderung), einzeln grün:**
  ...
  - **P1c Ressourcen-Meta & Daten** (in zwei Commits: **P1c-1** `meta.resources` + `resourceConfig` + Laden/Konflikte/Overwrite/Monatsfilter/actAs + `data`-Teil, erledigt; **P1c-2** Backend-Adapter (`meta.resources[].api`), `loadAllYearData`, `saveDaten`-Buttons, `ewt:deleted`, Monat/Jahr-Store, erledigt; `fieldMapper`/`dataApi`-Dateien und Storage-Enum bleiben statisch bis zu den Move-Phasen): `resourceConfig`, `Storage`, `persist*`, `mergeVisibleResourceRows`, `autoSave/*`, `warmeFormularCaches`, `loadUserDaten.*`, `overwriteUserDaten`, `changeMonatJahr`, `fieldMapper`, `actAs` aus `meta`/`data`-Slot; Neben, EWT, Bereitschaft migrieren (meta + lazy Modul); `setMonatJahr` → Monat/Jahr-Store bzw. `period:changed`.
