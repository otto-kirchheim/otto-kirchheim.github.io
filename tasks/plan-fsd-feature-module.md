...
- **P1 Feature-Contract & Module (XL, keine Moves, In-place, Verhaltensänderung), einzeln grün:**
  - **P1a Contract-Kern:** `FeatureMeta`/`FeatureModule`/`defineFeature`, Registry (`load`, `preload`, `activate`, Wake-Events, Fehlerpfad), `useFeatureModules`, Manifest `app/features.ts`, Vertragstests inkl. Lazy, `scripts/new-feature.ts` + Vorlage. **EA zuerst** (kleinster Slice) als Referenz-Migration (Tab mount lazy).
